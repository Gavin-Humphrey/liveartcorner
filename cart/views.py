...
def update_cart_total_cost(request):####
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            total_cost = cart.calculate_total_with_delivery(selected_delivery_method_id, discount_code)

            # Retrieve other relevant information for the summary
            cart_items = cart.get_cart_items()
            
            sub_total = cart.calculate_sub_total()
            chosen_delivery_method = get_object_or_404(DeliveryMethod, id=selected_delivery_method_id)  # Get the DeliveryMethod object###
            ...
            context = {
                ...
                'sub_total': sub_total,
                'chosen_delivery_method': chosen_delivery_method.method if chosen_delivery_method.method else None,  # Get the delivery method name
                'delivery_cost': chosen_delivery_method.cost if chosen_delivery_method.cost else 0,  
    ...
    return redirect('view-cart')  # Redirect back to the cart if no POST request or if there's an error

...
class ArtistDashboardViewTestCase(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create(username='testuser', email='test@example.com', password='password', is_artist=True)
...
class ManageItemsViewTestCase(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(username='testartist', email='test@example.com', password='password', is_artist=True)
...
class ManageItemsAvailabilityViewTestCase(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(username='testartist', email='test@example.com', password='password')
        ...
        image_io.seek(0)
        image_file = SimpleUploadedFile('img1.jpg', image_io.read(), content_type='image/jpeg')

        Item.objects.create(card=self.card, title='Item 1', image=image_file, length=10, width=5, is_available=True, price=100, quantity=1, description='Item 1 description', popularity=5)
        Item.objects.create(card=self.card, title='Item 2', image=image_file, length=20, width=15, is_available=False, price=200, quantity=2, description='Item 2 description', popularity=3)
    ...
    def test_manage_items_availability_view_no_card_items(self):
        user_without_card_items = User.objects.create_user(username='newuser', email='newuser@example.com', password='password')

...
def artist_dashboard(request, user_id):
    ...
    bookings = Booking.objects.filter(availability__artist=user).order_by('-availability__date')  
    ordered_items = OrderItem.objects.filter(item__card__user=user).select_related('order', 'item')

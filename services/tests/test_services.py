...
class YourTestCase(TestCase):
    def setUp(self):
        ...
        self.artist = User.objects.create_user(username='artistuser', email='artist@example.com', password='password')
    ...
    def test_service_booking_with_availability(self):
        ...
        response = self.client.post(reverse('service-booking', args=[self.service_with_availability.id]), {
    ...
    def test_service_booking_without_availability(self):
        ...
        response = self.client.post(reverse('service-booking', args=[self.service_without_availability.id]), {
        ...
        self.assertEqual(response.status_code, 404)  # Adjust as per your application's response

    def test_add_service(self):
        # Simulate authentication
        self.client.force_login(self.user)

        response = self.client.post(reverse('add-service'), {
            'name': 'New Service',
            'artist': self.user.id,
    ...
    def test_delete_service(self):
        ...
        response = self.client.post(reverse('delete-service', args=[self.service_with_availability.id]))
    ...
    def test_update_service(self):
        ...
        response = self.client.post(reverse('update-service', args=[self.service_with_availability.id]), {

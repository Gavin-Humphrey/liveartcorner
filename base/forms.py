...
class RegisterForm(UserCreationForm):
    ...
    phone_number = forms.CharField(required=False, validators=[
        RegexValidator(r'^\+\d{1,2}\s?[\d\-()]+\s?[\d\-()]+$', message='Invalid phone number format.')
    ])    
    street_address = forms.CharField(required=True, validators=[MinLengthValidator(2), RegexValidator(r'^[A-Za-z0-9\s\,\.]+$', message='Invalid characters in street address.')])
    city = forms.CharField(required=True, validators=[
    ...
    class Meta:
        model = User
        fields = ('name', 'email', 'phone_number', 'street_address', 'city', 'postal_code', 'country', 'password1', 'password2')

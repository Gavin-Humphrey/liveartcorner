...
def process_checkout(request):
    ...
    if request.method == 'POST':
        ...
        if delivery_form.is_valid():
            ...
            try:
                delivery_method_instance = DeliveryMethod.objects.get(method=chosen_delivery_method_name)
                logger.info(f"Delivery method instance: {delivery_method_instance}")

                order = create_order(request.user, delivery_data, delivery_method_instance)

...
class GuestUser(admin.TabularInline):
    ...
    extra = 0
    fields = ['is_user',]
    readonly_fields = ['is_user',]

class OrderAdmin(admin.ModelAdmin):
    model = Order
    extra = 0
    readonly_fields = ['user', 'order_type', 'items', 'total_cost', 'delivery_method', # Assuming no discount_code field
                       'order_status', 'id', 'created_at', 'updated_at']
    list_display = ['user', 'order_type', 'total_cost', 'delivery_method', # Assuming no discount_code field
                     'order_status', 'id', 'created_at', 'updated_at']
